...
ok('lists the pairs', /Suresh Babu[\s\S]*Ganesh Iyer/i.test(t))
...
await a.click('button:has-text("Pause it")')
await a.waitForURL(/\/more\?done=/, { timeout: 20000 })
t = await textOf(a)
...
await page.goto(`${BASE}/t/${MENS}`)
t = await textOf(page)
ok('the public page says it is paused', /Paused[\s\S]*Rain — back at three/i.test(t))
await shot(page, 'public-paused')
await page.goto(`${BASE}/`)
t = await textOf(page)
ok('and so does Today', /Paused[\s\S]*Men's Doubles — Rain — back at three/i.test(t))
await shot(page, 'today-paused')
await a.goto(`${BASE}/admin/t/${MENS}/more?do=pause`)
await a.click('button:has-text("Start again")')
await a.waitForURL(/\/more\?done=/, { timeout: 20000 })
await page.goto(`${BASE}/t/${MENS}`)
ok('resumed', !/Paused/.test(await textOf(page)))
