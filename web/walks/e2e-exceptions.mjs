...
const body = async (p = page) => { await settled(p); return p.evaluate(() => (document.querySelector('main') ?? document.body).innerText.replace(/\s+/g, ' ')) }
...
console.log('\n8. delete a not-started tournament (item 11)')
{
  ...
  ok('  the delete confirm counts its player and its courts coming free', /Its 1 player goes with it\. Court 1, Court 2 come free for the day\./.test(t), t.slice(0, 400))
