 ...
 *
 * Seeds its own tournaments through the Go seed (server/cmd/seed) unless SLUGS
 * is given as "doubles,singles,odd". Signs in with 482913, or with the temporary
...
if (process.env.SLUGS) {
  ...
  // The Go port's seed: the same three tournaments, printed the same way.
  // `go run` wants its own module, so it is run from server/.
  const out = execFileSync('go', ['-C', '../../server', 'run', './cmd/seed', 'teams'], { encoding: 'utf8' })
...
const body = async () => { await settled(page); return page.evaluate(() => (document.querySelector('main') ?? document.body).innerText.replace(/\s+/g, ' ')) }
let n = 0
...
ok('two pairs named each other', (t.match(/named each other/g) ?? []).length === 2)
...
ok('the note about splitting is there', /split again any time before the start/.test(t))
await shot('picker')
...
ok('mutual pairs survived', (t.match(/named each other/g) ?? []).length === 2)
...
console.log('\n11. make the schedule; a changed pair throws it away; the start locks the pairs')
...
await settle()
t = await body()
ok('pairs can still be changed before the start', /Split/.test(t) && !/Changing a pair is under More/.test(t), t.slice(0, 300))
// split one pair: the schedule made for the old pairs goes
await page.click('summary:has-text("Split")')
await page.waitForTimeout(200)
await page.click('details[open] button:has-text("Split them")')
await page.waitForTimeout(1500)
await settle()
t = await body()
ok('5 pairs made · 2 to pair', /5 pairs made · 2 players still to pair/.test(t), t.slice(0, 300))
await page.goto(`${D}/schedule`)
await settle()
t = await body()
ok('the stale schedule is gone', /schedule not made yet/.test(t) && /Make the schedule/.test(t), t.slice(0, 300))
await page.goto(`${D}/teams`)
await settle()
await page.click('summary:has-text("Pair the rest at random")')
await page.waitForTimeout(200)
...
await settle()
t = await body()
ok('the locked note shows once started', /The tournament has started\. Changing a pair is under More\./.test(t), t.slice(0, 300))
...
await settle()
t = await body()
ok('hub: singles has no Players step, Registration counts 6', !/in the draw/.test(t) && /6 players in/.test(t), t.slice(0, 400))
