...
const body = async (p = page) => { await settled(p); return p.evaluate(() => {
...
console.log('\n4. three people through the link')
{
  ...
  await p.waitForSelector('input[name=name]')
  t = await body(p)
  ok('the form: name, phone, partner, Sign me up', /Your name/.test(t) && /Phone · optional/.test(t) && /Playing with someone\?/.test(t) && /If you leave this blank the organiser pairs you up\./.test(t) && /Sign me up/.test(t))
...
console.log('\n6. the same phone under the same name is the same person')
const e = await signup('Arun Prakash', null, '9840012345', /You’re already on the list\./)
await e.fresh.close()
console.log('   …but under a different name it is a new row, flagged — a number never answers to a name')
const e2 = await signup('A Prakash', null, '9840012345', /You’re on the list\./)
await e2.fresh.close()
await page.goto(REG)
await settle()
t = await body()
ok('A Prakash is on with a flag against Arun', /6 in/.test(t) && /Same as Arun Prakash\?/.test(t), t.slice(0, 500))
await page.locator('li:has-text("A Prakash") button:has-text("Same person")').click()
t = await until(/A Prakash and Arun Prakash are one person/)
ok('merged back into Arun', /5 in/.test(t), t.slice(0, 300))
...
ok('the closed card', /Sign-ups closed/.test(t) && /Sign-ups have closed — ask the organiser/.test(t) && /Reopen sign-ups/.test(t))
