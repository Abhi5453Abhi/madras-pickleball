...
import { Card, CourtMark, CourtSwatch, Meter, Notice, StatusPill } from '@/components/ui'
...
type DashboardRow = Output<'events.dashboard'>['today'][number]
...
export function DashboardPage() {
  useTitle('Tournaments · Madras Pickleball')
  const [params] = useSearchParams()
  const denied = params.get('denied')
  const board = useRpc('events.dashboard', {})

  if (board.state === 'loading') return <Loading />
  if (board.state !== 'ready') return <LoadError error={board.error} retry={() => void board.reload(false)} />

  const { today, upcoming, finished } = board.data
    ...
    <div className="flex flex-col gap-7">
      ...
      {nothing ? (
        <div className="relative overflow-hidden rounded-card border border-line-strong bg-paper p-6 shadow-card">
          ...
          <Link to="/admin/new" className={`${PRIMARY_LINK} mt-5 max-w-[18rem]`}>
            Make a tournament
          </Link>
        </div>
      ) : null}
        ...
        <section className="flex flex-col gap-3">
          ...
          <Card>
            <ul className="divide-y divide-line">
              {finished.map((t) => (
                <li key={t.id}>
                  <Link to={`/admin/t/${t.slug}`} className="tap-lg flex items-center gap-3 px-4">
...
function TodayCard({ row: t }: { row: DashboardRow }) {
  ...
  return (
    <Link to={`/admin/t/${t.slug}`} className="block">
      <Card className="p-4">
  ...
  return (
    <Link to={`/admin/t/${t.slug}`} className="block">
      <Card className="p-4">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-section text-text">{t.name}</h3>
          <StatusPill state="waiting">{venueDate(t.day)}</StatusPill>
        </div>
        <p className="num mt-1 text-meta text-text-2">
          {categoryPrefix(t)}
        ...
        <p className="mt-1 text-meta text-text-3">
          ...
          {t.pendingSignups
            ? ` · ${t.pendingSignups} possible ${t.pendingSignups === 1 ? 'duplicate' : 'duplicates'}`
            : ''}
          {' · '}
          {t.courts.length ? t.courts.map((c) => c.name).join(', ') : 'no courts assigned yet'}
        </p>
      </Card>
    </Link>
  )
