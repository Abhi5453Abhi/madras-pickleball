...
await page.fill('input[name=text]', 'Arun Prakash 98400 12345')
...
ok('the box is empty again', (await page.inputValue('input[name=text]')) === '')
await page.fill('input[name=text]', 'arun prakash')
await page.click('button:has-text("Add")')
...
ok('the add-by-hand box is still there', (await page.getAttribute('input[name=text]', 'placeholder'))?.startsWith('Add a player by hand'))
...
await page.goto(REG)
await settle()
await page.fill('input[name=text]', 'Naveen Krishnan')
await page.click('button:has-text("Add")')
