...
await page.waitForLoadState('networkidle')
c = await cards()
ok('Court 3 says Mixed is paused — rain', /Mixed Doubles is paused — rain\./.test(c['Court 3'] ?? ''), c['Court 3'])
...
await page.click('[data-court]:has(.text-eyebrow:text-is("Court 3")) button:has-text("Start again")')
