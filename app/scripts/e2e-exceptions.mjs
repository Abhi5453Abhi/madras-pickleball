...
async function create({ name, gender, discipline, format, courts, date }) {
  ...
  await page.click(`fieldset:has(legend:text-is("Category")) button:has-text("${gender}")`)
...
console.log('\n   M3: a walkover — one side didn’t turn up (item 4)')
{
  ...
  ok('  Played shows it as a walkover', new RegExp(`${esc(M3.sides[0])} walkover over ${esc(M3.sides[1])}`).test(hub), hub.slice(0, 1200))
...
const W = M3.sides[1]
{
  ...
  ok('  every pair is listed with Tap', pairs.every((p) => t.includes(flat(p))) && /Tap/.test(t), t.slice(0, 600))
  ...
  const wo = (h.match(/walkover over/g) ?? []).length
...
console.log('\n   pause the day (item 8), then the horn on M4 while it is stopped')
{
  ...
  ok('  the pause screen explains itself and suggests a note', /Nothing is cancelled and nothing is lost/.test(t) && (await page.inputValue('#pause-note')) === 'Rain — back shortly', t.slice(0, 400))
  await page.fill('#pause-note', 'Rain — back in 20 minutes')
  await page.click('button:has-text("Stop the day")')
  const d = await until(/The day is stopped/)
  ok('  done: The day is stopped', /The day is stopped\. The public page says so\./.test(d), d.slice(0, 300))
  ...
  c = await cards()
  ok('  the board shows the paused line on the Men’s courts with Start again', /Men's Doubles is paused — Rain — back in 20 minutes\./.test(c['Court 1']) && /Start again/.test(c['Court 1']) && /Men's paused/.test(lb), `${c['Court 1']} | ${lb.slice(0, 300)}`)
  ok('  Court 3 (Mixed) is not paused', !/paused/.test(c['Court 3']), c['Court 3'])
  await shot('live-paused')
  const pub = await goto(`${BASE}/t/${mens.slug}`)
  ok('  the public page shows the stop notice', /The day is stopped/i.test(pub) && /Rain — back in 20 minutes/.test(pub), pub.slice(0, 400))
  await shot('public-paused')
  const today = await goto(`${BASE}/`)
  ok('  and Today does too, naming the tournament', /The day is stopped/i.test(today) && /Men's Doubles — Rain — back in 20 minutes/.test(today), today.slice(0, 400))
...
console.log('\n   swap a player while nothing is live (item 7)')
{
  ...
  await page.click('button:has-text("Make the swap")')
...
{
  await goto(`${BASE}/admin/live`)
  await page.click('[data-court]:has(.text-eyebrow:text-is("Court 1")) button:has-text("Start again")')
  ...
  const pub = await goto(`${BASE}/t/${mens.slug}`)
  ok('  the public page: no stop notice, the new name on court', !/The day is stopped/.test(pub) && pub.includes(sw.to.split(' / ')[1]), pub.slice(0, 800))
...
console.log('\n   put the withdrawn pair back (item 6)')
{
  ...
  const h = await goto(mens.hub)
  ok('  the hub’s Played is back to one walkover', (h.match(/walkover over/g) ?? []).length === 1, h.slice(0, 1200))
  const pub = await goto(`${BASE}/t/${mens.slug}`)
...
console.log('\n   changing a league score after the final (item 5)')
{
  ...
  await page.waitForURL(/\/admin\/m\//, { timeout: 20000 })
  await settle()
  // The format is one game to 11 now, so a re-entry of a best-of-3 match is one game.
...
console.log('\n5. Mixed — the table decides it (item 1)')
{
  ...
  const deadHeat = (h.match(/level — the organiser decides/g) ?? []).length
  ok('  the table says all three are level — the organiser decides', deadHeat === 3, h.slice(0, 700))
  ...
  if (deadHeat === 3 && new RegExp(`Winners ${esc(flat(winner ?? '@@'))}`, 'i').test(f)) {
    note(`BUG: Mixed ended in a three-way dead heat (each pair 1 win, 38 points) — the table said "level — the organiser decides" and there is nothing to decide it with, yet Finish crowned ${winner} as Winners anyway, in sign-up order.`)
  }
...
for (const name of ['Ganesh Iyer', 'Suresh Babu', 'Vijay Anand', 'Manoj Pillai', 'Kumar Raja']) await addByHand(singles.hub, name)
{
  ...
  ok('  hub: Players · 5 in the draw, done', /Players( — done)? 5 in the draw/.test(h), h.slice(0, 400))
