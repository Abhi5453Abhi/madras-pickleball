...
console.log('\n   pause the day (item 8), then the horn on M4 while it is stopped')
{
  ...
  c = await cards()
  ok('  the board says Men’s is stopped, once, with Start it again', /Men's Doubles is stopped/i.test(lb) && /Rain — back in 20 minutes/.test(lb) && /Start it again/.test(lb) && /Men's paused/.test(lb), lb.slice(0, 400))
...
{
  await goto(`${BASE}/admin/live`)
  await page.click('button:has-text("Start it again")')
