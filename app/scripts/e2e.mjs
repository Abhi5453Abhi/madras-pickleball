...
console.log('\n   two added by hand')
await page.fill('input[name=text]', 'Deepak Raj 98400 12345')
...
await page.fill('input[name=text]', 'Bala Murugan')
await page.click('button:has-text("Add")')
...
ok('the final is last, to be decided', /Final/.test(listed[6] ?? '') && /To be decided/.test(listed[6] ?? ''), listed[6])
ok('rounds are named in words', listed.filter((l) => /Round \d/.test(l)).length === 6, listed.join(' | '))
...
const top2 = rows.slice(0, 2).map((r) => r[1])
...
ok('the courts say every match has been played and point at the page', /Every Men's Doubles match has been played/.test(c['Court 1'] ?? '') && /Finish it on its page/.test(c['Court 1'] ?? ''), c['Court 1'])
await shot('live-all-played')

console.log('\n   finish')
await page.click('[data-court] a:has-text("Finish it on its page")')
await page.waitForURL(new RegExp(`/admin/t/${slug}$`), { timeout: 30000 })
await settle()
t = await body()
